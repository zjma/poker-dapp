module contract_owner::poker_room_examples {
    ...
    #[test_only]
    use contract_owner::poker_room::{get_room_brief, state_update, process_shuffle_contribution,
        process_dkg_contribution, join, create, cur_game,
        process_private_dealing_reencryption, process_private_dealing_contribution, process_public_opening_contribution,
        process_new_bet, cur_dkg, cur_shuffle, is_in_game, process_showdown_reveal
    };
    #[test_only]
    ...
    fun example(framework: signer, host: signer) {
        ...
        let (dkg_0_eric_secret_share, dkg_0_eric_contribution) = dkg_v0::generate_contribution(cur_dkg(&room));
        ...
        let (dkg_0_alice_secret_share, dkg_0_alice_contribution) = dkg_v0::generate_contribution(cur_dkg(&room));
        ...
        let (dkg_0_bob_secret_share, dkg_0_bob_contribution) = dkg_v0::generate_contribution(cur_dkg(&room));
        ...
        let game_0_alice_shuffle_contri = shuffle::generate_contribution_locally(&alice, cur_shuffle);
        ...
        let game_0_bob_shuffle_contri = shuffle::generate_contribution_locally(&bob, cur_shuffle);
        ...
        let game_0_eric_shuffle_contri = shuffle::generate_contribution_locally(&eric, cur_shuffle);
        ...
        let (game_0_deal_0_alice_secret, game_0_deal_0_alice_reenc) = reencryption::reencrypt(&alice, game::borrow_private_dealing_session(cur_game, 0));
        let (game_0_deal_1_alice_secret, game_0_deal_1_alice_reenc) = reencryption::reencrypt(&alice, game::borrow_private_dealing_session(cur_game, 1));
        let (game_0_deal_2_bob_secret, game_0_deal_2_bob_reenc) = reencryption::reencrypt(&bob, game::borrow_private_dealing_session(cur_game, 2));
        let (game_0_deal_3_bob_secret, game_0_deal_3_bob_reenc) = reencryption::reencrypt(&bob, game::borrow_private_dealing_session(cur_game, 3));
        let (game_0_deal_4_eric_secret, game_0_deal_4_eric_reenc) = reencryption::reencrypt(&eric, game::borrow_private_dealing_session(cur_game, 4));
        let (game_0_deal_5_eric_secret, game_0_deal_5_eric_reenc) = reencryption::reencrypt(&eric, game::borrow_private_dealing_session(cur_game, 5));
        ...
        assert!(is_in_game(&room, 0), 999);
        assert!(vector[false, false, false] == game::get_fold_statuses(cur_game(&room)), 999);
        assert!(game::is_phase_1_betting(cur_game(&room), alice_addr), 999);
        print(&game::get_bets(cur_game(&room)));
        assert!(vector[0, 125, 250] == game::get_bets(cur_game(&room)), 999);

        print(&utf8(b"Alice takes a look at her private cards."));
        let game_0_alice_card_0 = game::reveal_dealed_card_locally(&alice, cur_game(&room), 0, game_0_deal_0_alice_secret);
        let game_0_alice_card_1 = game::reveal_dealed_card_locally(&alice, cur_game(&room), 1, game_0_deal_1_alice_secret);
        ...
        let game_0_bob_card_0 = game::reveal_dealed_card_locally(&bob, cur_game(&room), 2, game_0_deal_2_bob_secret);
        let game_0_bob_card_1 = game::reveal_dealed_card_locally(&bob, cur_game(&room), 3, game_0_deal_3_bob_secret);
        ...
        let game_0_eric_card_0 = game::reveal_dealed_card_locally(&eric, cur_game(&room), 4, game_0_deal_4_eric_secret);
        let game_0_eric_card_1 = game::reveal_dealed_card_locally(&eric, cur_game(&room), 5, game_0_deal_5_eric_secret);
        ...
        assert!(shuffle::is_waiting_for_contribution(cur_shuffle(&room), alice_addr), 999);
        let game_1_alice_shuffle_contri = shuffle::generate_contribution_locally(&alice, cur_shuffle(&room));
        ...
        let game_1_bob_shuffle_contri = shuffle::generate_contribution_locally(&bob, cur_shuffle(&room));
        ...
        assert!(shuffle::is_waiting_for_contribution(cur_shuffle(&room), eric_addr), 999);
        let game_1_eric_shuffle_contri = shuffle::generate_contribution_locally(&eric, cur_shuffle(&room));
        ...
        assert!(vector[0, 125, 250] == game::get_bets(cur_game(&room)), 999);
        assert!(vector[true, false, false] == game::get_fold_statuses(cur_game(&room)), 999);
        ...
        let room = get_room_brief(host_addr);
        assert!(is_in_game(&room, 0), 999);
        assert!(vector[0, 500, 250] == game::get_bets(cur_game(&room)), 999);
        assert!(vector[true, false, false] == game::get_fold_statuses(cur_game(&room)), 999);
        ...
        let room = get_room_brief(host_addr);
        assert!(is_in_game(&room, 0), 999);
        assert!(vector[0, 500, 500] == game::get_bets(cur_game(&room)), 999);
        assert!(vector[true, false, false] == game::get_fold_statuses(cur_game(&room)), 999);
        ...
        for_each(vector[0,1,2], |opening_idx|{
            let scalar_mul_session = game::borrow_public_opening_session(cur_game(&room), opening_idx);
