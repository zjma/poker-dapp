module contract_owner::product_argument {
    ...
    public fun encode_proof(proof: &Proof): vector<u8> {
        ...
        vector::for_each_ref(&proof.vec_a_tilde, |scalar|{
        ...
        vector::for_each_ref(&proof.vec_b_tilde, |scalar|{
            vector::append(&mut buf, group::encode_scalar(scalar));
        });
    ...
    public fun prove(
        ...
        vector::append(&mut vec_delta, vector::map(vector::range(1, n-1), |_|group::rand_scalar()));
        ...
        let vec_2 = vector::map(vector::range(0, n-1), |i|{ group::scalar_neg(&group::scalar_mul(&vec_d[i+1], &vec_delta[i])) });
        ...
        let r_tilde = group::scalar_add(
            &group::scalar_mul(&x, r),
            &r_d,
        );
        let s_tilde = group::scalar_add(
            &group::scalar_mul(&x, &s_x),
            &s_1
        );
    ...
    public fun verify(
        ...
        if (group::element_add(&group::scale_element(vec_a_cmt, &x), &proof.vec_d_cmt)
            !=
            pederson_commitment::vec_commit(pederson_ctxt, &proof.r_tilde, &proof.vec_a_tilde)
        ) return false;

        let tmp_vec = vector::map(vector::range(0, n-1), |i|{
            group::scalar_sub(
