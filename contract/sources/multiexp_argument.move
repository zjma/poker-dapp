module contract_owner::multiexp_argument {
    ...
    public fun encode_proof(proof: &Proof): vector<u8> {
        ...
        vector::for_each_ref(&proof.a_vec, |val|{
    ...
    public fun prove(
        ...
        vec_c: &vector<elgamal::Ciphertext>, c: &elgamal::Ciphertext, vec_a_cmt: &group::Element,
        vec_a: &vector<group::Scalar>, r: &group::Scalar, rho: &group::Scalar,
    ): Proof {
        ...
        let b_cmt_vec = vector::map(vector::range(0, 2), |k|pederson_commitment::vec_commit(pedersen_ctxt, &s_vec[k], &vector[b_vec[k]]));
        let e_vec = vector::map(vector::range(0, 2), |k|{
        ...
        fiat_shamir_transform::append_raw_bytes(trx, elgamal::encode_ciphertext(&e_vec[0]));
        fiat_shamir_transform::append_group_element(trx, &b_cmt_vec[1]);
        fiat_shamir_transform::append_raw_bytes(trx, elgamal::encode_ciphertext(&e_vec[1]));

        let x = fiat_shamir_transform::hash_to_scalar(trx);
        let a_out_vec = vector::map(vector::range(0, n), |i|{
            group::scalar_add(
                &vec_a_0[i],
        ...
        let tau_out = group::scalar_add(&tau_vec[0], &group::scalar_mul(&tau_vec[1], &x));
        Proof {
            cmt_a0: vec_a_0_cmt,
            b_cmt_0: b_cmt_vec[0],
            b_cmt_1: b_cmt_vec[1],
    ...
    public fun verify(
        ...
        let (enc_base, _) = elgamal::unpack_enc_key(*ek);
        let b_0_cmt_expected = pederson_commitment::vec_commit(pedersen_ctxt, &group::scalar_from_u64(0), &vector[]);
        fiat_shamir_transform::append_group_element(trx, &proof.cmt_a0);
        fiat_shamir_transform::append_group_element(trx, &proof.b_cmt_0);
        fiat_shamir_transform::append_raw_bytes(trx, elgamal::encode_ciphertext(&proof.e_0));
        fiat_shamir_transform::append_group_element(trx, &proof.b_cmt_1);
        fiat_shamir_transform::append_raw_bytes(trx, elgamal::encode_ciphertext(&proof.e_1));
        let x = fiat_shamir_transform::hash_to_scalar(trx);
        ...
        if (group::element_add(&proof.b_cmt_0, &group::scale_element(&proof.b_cmt_1, &x))
            !=
            pederson_commitment::vec_commit(pedersen_ctxt, &proof.s, &vector[proof.b])
        ) return false;

        if (elgamal::ciphertext_add(&proof.e_0, &elgamal::ciphertext_mul(&proof.e_1, &x))
            !=
            elgamal::ciphertext_add(
                &elgamal::enc(ek, &proof.tau, &group::scale_element(&enc_base, &proof.b)),
    ...
    fun completeness(framework: signer) {
        ...
        let proof = prove(&ek, &pedersen_ctxt, &mut trx_prover, &vec_c, &c, &vec_a_cmt, &vec_a, &r, &rho);
        assert!(verify(&ek, &pedersen_ctxt, &mut trx_verifier, &vec_c, &c, &vec_a_cmt, &proof), 999);
    }
}