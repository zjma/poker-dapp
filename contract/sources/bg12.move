module contract_owner::bg12 {
    ...
    public fun prove(
        ...
        original: &vector<elgamal::Ciphertext>, shuffled: &vector<elgamal::Ciphertext>,
        permutation: vector<u64>, vec_rho: &vector<group::Scalar>,
    ): Proof {
        ...
        let vec_b = vector::map(vector::range(0, n), |i|x_powers[permutation[i]]);
        ...
        let vec_neg_z_cmt = pederson_commitment::vec_commit(pedersen_ctxt, &group::scalar_from_u64(0), &vec_neg_z);
        let vec_d_cmt = group::element_add(
            &group::scale_element(&vec_a_cmt, &y),
            &vec_b_cmt,
        );
        let vec_d = vector::map(vector::range(0, n), |i|{
            group::scalar_add(
                &group::scalar_mul(&y, &vec_a[i]),
                &vec_b[i],
            )
        });
        let t = group::scalar_add(&group::scalar_mul(&y, &r), &s);

        let tmp_vec = vector::map(vector::range(0, n), |i| group::scalar_sub(&vec_d[i], &z));
        let tmp_product = group::scalar_from_u64(1);
        vector::for_each_ref(&tmp_vec, |v|{
            tmp_product = group::scalar_mul(&tmp_product, v);
        });
        let trx_branch = *trx;
        let product_proof = product_argument::prove(pedersen_ctxt, &mut trx_branch, n, &group::element_add(&vec_d_cmt, &vec_neg_z_cmt), &tmp_product, &tmp_vec, &t);
        ...
        let multiexp_proof = multiexp_argument::prove(ek, pedersen_ctxt, trx, shuffled, &tmp_ciph, &vec_b_cmt, &vec_b, &s, &group::scalar_neg(&rho));
        Proof {
            vec_a_cmt,
            vec_b_cmt,
            multiexp_proof,
    ...
    public fun verify(
        ...
        let trx_branch = *trx;
        let tmp_ciph = elgamal::weird_multi_exp(original, &x_powers);
        product_argument::verify(pedersen_ctxt, &mut trx_branch, n, &group::element_add(&vec_d_cmt, &vec_neg_z_cmt), &tmp_product, &proof.product_proof)
            && multiexp_argument::verify(ek, pedersen_ctxt, trx, shuffled, &tmp_ciph, &proof.vec_b_cmt, &proof.multiexp_proof)
    }
    ...
    fun completeness(framework: signer) {
        ...
        let old_deck = vector::map(vector::range(0, n), |i| elgamal::enc(&ek, &group::rand_scalar(), &card_plaintexts[i]));

        let rerandomizers = vector::map(vector::range(0, n), |_|group::rand_scalar());
        let permutation = randomness::permutation(n);
        let new_deck = vector::map(vector::range(0, n), |i|{
        ...
        let proof = prove(&ek, &pedersen_ctxt, &mut trx, &old_deck, &new_deck, permutation, &rerandomizers);
        assert!(verify(&ek, &pedersen_ctxt, &mut trx_verifier, &old_deck, &new_deck, &proof), 999);
    }
}
