module contract_owner::poker {
    ...
    fun start_round(room: &mut GameRoomState) {
        debug::print(&utf8(b"start_round: BEGIN"));
        let player_addrs = vector[];
        let player_chips = vector[];
        let new_btn_pos = 0xffff;
        let n = 0;
        let pos = room.last_button_position;
        while (true) {
            pos = next_alive_player(room, pos);
        ...
        assert!(vector::is_empty(&errors), 161604);
        ...
        debug::print(&utf8(b"start_round: END"));
    }
    ...
    #[randomness]
    entry fun join(player: &signer, game_addr: address, ek_bytes: vector<u8>) acquires GameRoomState {
    ...
    #[randomness]
    entry fun process_dkg_contribution(player: &signer, game_addr: address, session_id: u64, contribution_bytes: vector<u8>, proof_bytes: vector<u8>) acquires GameRoomState {
        debug::print(&utf8(b"process_dkg_contribution: BEGIN"));
        let room = borrow_global_mut<GameRoomState>(game_addr);
        ...
        if (status == 1) { // Finished!
            ...
            abort(20250306162010);
        };
        debug::print(&utf8(b"process_dkg_contribution: END"));
    }

    #[randomness]
    entry fun process_shuffle_contribution(player: &signer, game_addr: address, session_id: u64, shuffle_result_bytes: vector<u8>, proof_bytes: vector<u8>) acquires GameRoomState {
        debug::print(&utf8(b"poker::process_shuffle_contribution: BEGIN"));
        let room = borrow_global_mut<GameRoomState>(game_addr);
        ...
        assert!(vector::is_empty(&errors), 180924);
        if (major_state_change == 1) { // Finished!
            // should no happen
            assert!(vector::is_empty(&errors), 180925);
        ...
        debug::print(&utf8(b"poker::process_shuffle_contribution: END"));
    }
    ...
    fun end_to_end(framework: signer, host: signer, alice: signer, bob: signer, eric: signer) acquires GameRoomState {
        ...
        assert!(round::is_waiting_for_shuffle_contribution_from(&room.cur_round, alice_addr), 999);
        let deck = round::borrow_deck(&room.cur_round);
        ...
        // Bob follows.
        let room = get_room_state(host_addr);
        assert!(round::is_waiting_for_shuffle_contribution_from(&room.cur_round, bob_addr), 999);
        let deck = round::borrow_deck(&room.cur_round);
        ...
        // Eric concludes.
        let room = get_room_state(host_addr);
        assert!(round::is_waiting_for_shuffle_contribution_from(&room.cur_round, eric_addr), 999);
        let deck = round::borrow_deck(&room.cur_round);
